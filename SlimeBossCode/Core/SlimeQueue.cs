...
public static class SlimeQueue
{
    ...
    private static List<Creature> GetSlimes(Player player) =>
        player.PlayerCombatState?.Pets.Where(e => e.Monster is SlimeModel).ToList() ?? [];
    ...
    public static Task AddSlime<T>(Player player) where T : SlimeModel => AddSlime(player, ModelDb.Monster<T>());
    ...
    private static void RearrangeSlimeOrbRow(Player player)
    {
        ...
        if (totalRequestedTSpan > 1.0f && totalSlimes > 1)
        {
            tEnd = 1.0f;
        }
        ...
        for (var i = 0; i < totalSlimes; i++)
        {
            ...
            if (player.Creature.Side == CombatSide.Enemy)
            {
