...
public static class SneckoPoolSelection
{
        ...
        Player snecko, IRunState state, uint[] choiceIds)
    {
        ...
        var chosenCharacters = await SyncSelections(snecko, sixCharacters, selectScene, choiceIds);
        ...
        uint[] choiceIds)
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            var index = await SyncOneChoice(snecko, left, right, selectScene, choiceIds[i]);
