...
public class CardExport : ItemExport, IImageExport
{
    ...
    [JsonInclude] [JsonPropertyName("id")] public virtual string Id => _model.Id.Entry;
    ...
    public string Color =>
        _model.VisualCardPool is ICustomEnergyIconPool && _model.VisualCardPool.EnergyColorName != "colorless"
            ? _model.VisualCardPool.EnergyColorName.ToLower()
            : _model.VisualCardPool.Title.ToLower();
    ...
    [JsonIgnore]
    private CardExport? UpgradedVersion => Upgrades >= _model.MaxUpgradeLevel
        ? null
        : new CardExport(_model.CanonicalInstance, Upgrades + 1);

    private string UpgradeDesc => UpgradedVersion is { } up ? up.Description : Description;

    [JsonIgnore]
    public string TextAndUpgrade =>
        ProcessCombinedDescription(CombineDescriptions(Description, UpgradeDesc, TextMode.Normal), TextMode.Normal);

    [JsonIgnore]
    public string TextWikiData =>
        ProcessCombinedDescription(CombineDescriptions(Description, UpgradeDesc, TextMode.WikiData), TextMode.WikiData);

    [JsonIgnore]
    public string TextWikiFormat =>
        ProcessCombinedDescription(CombineDescriptions(Description, UpgradeDesc, TextMode.WikiFormat),
            TextMode.WikiFormat);
    ...
    public string? StarCost => _model.HasStarCostX ? "X" :
        _model.CanonicalStarCost == -1 ? null : _model.CanonicalStarCost.ToString();

    [JsonInclude]
    [JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
    [JsonPropertyName("tinkerTimeRider")]
    ...
    public ViewportManager.DrawRequest[] ExportImg(ExportConfig config)
    {
        ...
        ViewportManager.DrawRequest Request()
        {
            return new ViewportManager.DrawRequest(ImgSize,
                $"card-images/{(Upgrades == 0 ? Id : $"{Id}Plus{Upgrades}")}", null, drawer =>
                {
    ...
    private static string CombineDescriptions(string a, string b, TextMode mode)
    {
        ...
        for (var aI = 0; aI <= aWords.Length; aI++) score[aI, 0] = aI * insertA;
        for (var bI = 0; bI <= bWords.Length; bI++) score[0, bI] = bI * insertB;
        ...
        if (mode == TextMode.Normal || mode == TextMode.WikiFormat)
        {
            ...
            {
                builder.Append(" (not");
                for (var j = astart; j < words.Count; j++)
                {
                    builder.Append(' ');
                    builder.Append(words[j]);
                }
    ...
    private string ProcessCombinedDescription(string description, TextMode textMode)
    {
        ...
        do
        {
            ...
            description = (start > 0 ? description[..start] : "") +
                          (textMode == TextMode.Normal ? "<span style=\"color:" + code + "\">" : "") +
                          description[(start + 9)..];
