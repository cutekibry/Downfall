...
public static class MasterPatchOnPlayWrapper
{
        ...
        MethodBase original)
    {
        ...
        var code = AsyncMethodCall.Create(generator, instructions, original,
            callMethod: AccessTools.Method(typeof(MasterPatchOnPlayWrapper), nameof(BeforeOnPlay)),
            beforeState: onPlayMethod,
            resultName: "returnIf"); 

        code = AsyncMethodCall.Create(generator, code, original,
            callMethod: AccessTools.Method(typeof(MasterPatchOnPlayWrapper), nameof(AfterOnPlay)),
            afterState: onPlayMethod);

        return code;
    }
    
    public static async Task<bool> BeforeOnPlay(CardModel __instance, PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        foreach (var cb in CardExecutionRegistry.BeforeListeners)
            if (!await cb(__instance, choiceContext, cardPlay)) return true;
        return false;
    }
